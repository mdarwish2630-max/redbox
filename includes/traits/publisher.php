...
trait PublisherTrait
{
  ...
  public function publisher($args = [])
  {
    ...
    /* set post category (RedNote browse categories) */
    if (!empty($args['category_id'])) {
      try {
        $db->query(sprintf("UPDATE posts SET category_id = %s WHERE post_id = %s", secure($args['category_id'], 'int'), secure($post['post_id'], 'int')));
