...
trait PostsTrait
{
  ...
  public function get_posts($args = [])
  {
    ...
    /* RedNote: filter by browse category */
    $category_id = !isset($args['category_id']) ? 0 : (int)$args['category_id'];
    if ($category_id > 0) {
      $where_query .= sprintf(" AND posts.category_id = %s", secure($category_id, 'int'));
    }
    /* filter posts */
    if ($filter != "all") {
