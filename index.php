...
try {
  ...
  if (!$user->_logged_in) {
    ...
    if (!$system['newsfeed_public']) {
      ...
      if (!empty($_GET['category'])) {
        $get_posts_args['category_id'] = (int)$_GET['category'];
      }
      $smarty->assign('posts', $user->get_posts($get_posts_args));
    ...
    switch ($_GET['view']) {
      case '':
        ...
        if ($system['newsfeed_merge_enabled']) {
          ...
          $posts = $user->get_posts($get_posts_args);
        }
  ...
  try {
    ...
    if ($get_browse_categories->num_rows > 0) {
      ...
      while ($category = $get_browse_categories->fetch_assoc()) {
        ...
        // only first 7 for header bar (recommended)
        if ($counter < 7) {
  ...
  $smarty->assign('header_categories', $header_categories);
