  ...
  <div class="float-end">
      ...
      <div class="dropdown-menu dropdown-menu-end countries-dropdown">
        <div class="js_scroller">
          ...
          {foreach $countries as $country}
            <a class="dropdown-item" href="?country={$country['country_name_native']}">{$country['country_name']}</a>
          {/foreach}
        </div>
      </div>
    {/if}
    ...
    {elseif $_filter == "article"}
      {if $user->_data['can_write_blogs']}
        <a href="{$system['system_url']}/blogs/new" class="btn btn-sm btn-primary">{__("Create Blog")}</a>
      {/if}
    {elseif $_filter == "product" && !$_query}
      {if $system['market_enabled'] && !in_array($_get, ['posts_page', 'posts_group', 'posts_event'])}
        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-url="posts/product.php?do=create">{__("Create Product")}</button>
      {/if}
    {elseif $_filter == "funding"}
      {if $user->_data['can_raise_funding'] && !in_array($_get, ['posts_page', 'posts_group', 'posts_event'])}
        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-url="posts/funding.php?do=create">{__("Create Funding")}</button>
      {/if}
    {elseif $_filter == "offer"}
      {if $system['offers_enabled'] && !in_array($_get, ['posts_page', 'posts_group', 'posts_event'])}
        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-url="posts/offer.php?do=create">{__("Create Offer")}</button>
      {/if}
    {elseif $_filter == "job"}
      {if $system['jobs_enabled'] && !in_array($_get, ['posts_page', 'posts_group', 'posts_event'])}
        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-url="posts/job.php?do=create">{__("Create Job")}</button>
      {/if}
    {elseif $_filter == "course"}
      {if $user->_data['can_create_courses'] && !in_array($_get, ['posts_page', 'posts_group', 'posts_event'])}
        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-url="posts/course.php?do=create">{__("Create Course")}</button>
      {/if}
    {/if}
  </div>
</div>
...
<div class="js_posts_stream" data-get="{$_get}" data-filter="{if $_filter}{$_filter}{else}all{/if}" data-country="{if $selected_country}{$selected_country['country_id']}{else}all{/if}" {if $_id}data-id="{$_id}" {/if} {if $_query}data-query="{$_query}" {/if}>
  {if $posts}
    ...
    <!-- see-more -->
    <div class="rednote-see-more see-more js_see-more js_see-more-infinite" data-get="{if $_load_more}{$_load_more}{else}{$_get}{/if}" data-filter="{if $_filter}{$_filter}{else}all{/if}" data-country="{if $selected_country}{$selected_country['country_id']}{else}all{/if}" {if $_id}data-id="{$_id}" {/if} {if $_query}data-query="{$_query}" {/if}">
